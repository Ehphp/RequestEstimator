...
import { getLists, saveList, deleteList, getRequirementsByListId, getLatestEstimate } from '../lib/storage';
...
export function ListsView({ onSelectList }: ListsViewProps) {
  ...
  const getListStats = (listId: string) => {
    ...
    const totalDays = requirements.reduce((sum, req) => {
      const estimate = getLatestEstimate(req.req_id);
      return sum + (estimate?.total_days || 0);
    }, 0);
    
    return { totalRequirements, totalDays };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {lists.map((list) => {
          const stats = getListStats(list.list_id);
